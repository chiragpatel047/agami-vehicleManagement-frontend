...
const AdminPendingRequests = () => {
    ...
    const fetchMaintenances = async () => {
      try {
        const response = await api.get(
          `/maintenanceByStatus?status=Pending`
        );
    ...
    const goToAssignment = async (user_id,record_id, e) => {
      e.preventDefault();

      navigate("/SelectMechanic?record_id="+record_id+"&user_id="+user_id);
                ...
                {records.length > 0 ? (
                  records.map((record, index) => (
                    <tr class="hover:bg-gray-100">
                      ...
                      <td class="px-2 py-4">
                        <button
                          onClick={(e) => goToAssignment(record.user_id,record.record_id, e)}
